...
@Component({
    selector: 'jhi-driver-vma',
    templateUrl: './driver-vma.component.html'
})
export class DriverVmaComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.driverService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<DriverVma[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/driver-vma'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/driver-vma', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: DriverVma) {
        return item.id;
    }
