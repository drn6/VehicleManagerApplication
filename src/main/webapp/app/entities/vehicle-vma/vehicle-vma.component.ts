...
@Component({
    selector: 'jhi-vehicle-vma',
    templateUrl: './vehicle-vma.component.html'
})
export class VehicleVmaComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.vehicleService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<VehicleVma[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/vehicle-vma'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/vehicle-vma', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: VehicleVma) {
        return item.id;
    }
